(() => {
  ...
  function validateConfig() {
    ...
    if (!C.ADMIN_EMAIL || C.ADMIN_EMAIL.includes('votre-adresse')) {
      issues.push('ADMIN_EMAIL');
    }

    if (!C.GOOGLE_CLIENT_ID || C.GOOGLE_CLIENT_ID.includes('VOTRE_CLIENT')) {
      issues.push('GOOGLE_CLIENT_ID');
    }

    if (!C.APPS_SCRIPT_URL || C.APPS_SCRIPT_URL.includes('VOTRE_DEPLOIEMENT')) {
      issues.push('APPS_SCRIPT_URL');
    }

    if (!C.PUBLIC_BASE_URL || C.PUBLIC_BASE_URL.includes('votrecompte')) {
    ...
    const timer = setInterval(() => {
      ...
      if (attempts > 100) {
        ...
        showAlert(
          authAlert,
          'error',
          'Le service de connexion Google n’a pas pu être chargé. Rechargez la page.'
        );
      }
    }, 100);
  }
  ...
  function signIn() {
    ...
    // La demande OAuth est uniquement déclenchée par le clic utilisateur.
    // Cela évite les blocages liés aux demandes silencieuses automatiques.
  ...
  function clearPendingTokenRequest_() {
  ...
  function handleTokenError(err) {
    ...
    const message =
      ...
      type +
      '. Cliquez de nouveau sur « Connexion Google ».';
  ...
  async function ensureFreshToken() {
    ...
    throw new Error(
      'Votre session Google a expiré. Cliquez sur « Connexion Google » puis reprenez le transfert.'
    );
  }
  ...
  async function createTransfer() {
    ...
    try {
      ...
      const transferUrl =
        `${base}/transfer/?id=${encodeURIComponent(
          token
        )}`;
...
async function sendMetadata(payload) {
  ...
  async function uploadResumable(
    ...
    if (
      ...
      throw new Error(
        'Votre session Google a expiré. Cliquez sur « Connexion Google » puis recommencez le transfert.'
      );
    }
